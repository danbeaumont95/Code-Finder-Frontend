...
function CodeContainer({
  codeSnippets, selectOption, searchText, isDashboard,
}: Props) {
  ...
  const formatData = (e: CodeSnippet[]) => {
    if (isDashboard) {
      ...
      if (selectOption !== '' && selectOption !== undefined && (searchText === '' || searchText === undefined)) {
        const mappedData = onlyUser
          .filter((el) => el.language.toLowerCase() === selectOption.toLowerCase());
        return mappedData;
      }

      if (searchText !== '' && searchText !== undefined) {
        const mappedData = onlyUser
          .filter((el: any) => el.title.toLowerCase().includes(searchText.toLowerCase()));

        if (selectOption !== '' && selectOption !== undefined) {
          const languageFiltered = mappedData
            .filter((el) => el.language.toLowerCase() === selectOption.toLowerCase());
    ...
    if (searchText !== '' && searchText !== undefined) {
      const mappedData = e.filter((el: any) => el.title.toLowerCase()
      ...
      {formatData(codeSnippets).map((el) => (
        <div className="code_snippet_container">
          ...
          <div className="code_snippet_button_container">
            <button type="button" className="login_here_button" onClick={handleSaveCodeSnippet} id={el.id.toString()}>Save</button>
          </div>
