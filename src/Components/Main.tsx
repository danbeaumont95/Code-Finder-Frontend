...
interface Props extends PropsFromRedux {
  register: (user: UserToSignUp) => void;
}
...
const Main = (props: Props) => {
  ...
  const [user, setUser] = useState<UserToSignUp>({first_name: '', last_name: '', email: '', password: ''})
      ...
      <div className='main_container'>
        ...
        <div className='main_container_form'>
          {type === 'login' ? (
            ...
            <div className='switch_type_section'>
              <h3 style={{ paddingTop: '20px'}}>Already have an account? <button className="login_here_button" onClick={() => setType('login')}>Login here</button></h3>
              <button type='submit' className='form_button' >Sign up!</button>
            </div>
